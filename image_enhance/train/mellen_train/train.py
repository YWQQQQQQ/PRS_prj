...
def my_loss(y_true, y_pred):
    ...
    SSIM_loss = utils.tf_ssim(tf.expand_dims(y_pred[:, :, :, 0], -1),tf.expand_dims(y_true[:, :, :, 0], -1)) + utils.tf_ssim(
        tf.expand_dims(y_pred[:, :, :, 1], -1), tf.expand_dims(y_true[:, :, :, 1], -1)) + utils.tf_ssim(
...
fake_B = mbllen(img_A)
vgg_fake = Lambda(utils.range_scale)(fake_B)
fake_features = vgg(vgg_fake)
fake_features = Lambda(f3)(fake_features)

img_B_vgg = Lambda(utils.range_scale)(img_B)
...
combined.compile(loss=my_loss,
                 metrics=[utils.bright_mae, utils.bright_mse, utils.bright_psnr, utils.bright_SSIM, utils.bright_AB],
                 optimizer=opt)
